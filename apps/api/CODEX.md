...
上传和重跑 ASR 默认会先生成完整备份 `audio-raw.wav`，再把 `audio.wav` 裁剪为只包含人声/非静音片段的短音频；同目录也会写出 `audio-vad.wav` 和 `audio-vad-map.json`，用于 Whisper 转写和原视频时间轴映射。

黑盒验证：
1. 上传或重试包含长停顿的视频 -> 验证: 视频目录存在完整 `audio-raw.wav`，且 `audio.wav` / `audio-vad.wav` 明显更短。
