...
旧版文档已备份到 `docs/archive/pre-v1.0-interview-docs-20260610/06-implemented-features-tech-doc.md`。当前 1.0 完整功能以 `docs/v1.0/feature-implementation-map.md` 和 `docs/v1.0/full-interview-question-bank.md` 为主索引。相比旧版，1.0 需要重点补充以下已经接通的能力：

- Docker 默认后端模式：MySQL、Redis、Qdrant。
- DeepSeek 作为 Chat LLM，Provider 可保存、启用、测试。
- Embedding Provider 与 LLM 解耦，Qdrant 作为外部向量库，local hash fallback 保证演示。
- Agent trace 集成输入检查、多轮记忆、字幕检索、向量检索、rerank、引用生成、LLM 调用、置信度判断和持久化。
- 多视频知识库管理：创建、删除、添加视频、移除视频、跨视频问答、观点对比、引用跳转。
- ASR 精度优化：简体化、乱码修复、技术词纠错、OCR/ASR 对齐、低置信片段修复、术语词库。
- 诊断台：Runtime、MySQL EXPLAIN、Redis Key、JVM 线程池、ASR 产物、Qdrant 向量状态、失败任务恢复。
- URL 导入保持 MVP，平台反爬和浏览器插件进入 2.0，不作为 1.0 主线。

1.0 对外口径：

```text
OmniVid 1.0 是可运行、可演示、可面试追问的闭环版本。它不是生产级 SaaS，也不声称解决平台反爬、多租户权限、真实 PPT 文件导出和云部署；这些都归入 2.0。
```
...
| ffmpeg 抽音频 + VAD | 已实现 | 上传后先生成 `audio-raw.wav`，再裁剪 `audio.wav` / `audio-vad.wav` 供 ASR 转写 | Java 调系统进程、静音检测、超时、日志文件 |
| whisper.cpp ASR | 已实现 | 字幕区展示真实 ASR 片段，并可映射回原视频时间轴 | whisper-cli + tiny 模型 + JSON 解析 + VAD 映射 |
...
| 当前视频 Agent | 已实现 | 可针对当前视频提问，并返回引用时间戳 | 字幕检索 + Qdrant 向量召回 + 本地 rerank + 可配置 LLM 证据约束回答 |
| 知识库 Agent | 已实现 | 可创建知识库、添加视频并跨视频对比观点 | 多视频字幕聚合 + Qdrant 向量召回 + 引用跳转 + 可配置 LLM |
...
| 云端 LLM | 已接入 DeepSeek Chat | 页面保存 Key 后总结和 Agent 回答会优先调用 OpenAI-compatible `/chat/completions`，失败自动降级 |
...
后端从上传视频中抽取完整 WAV 音频，再通过静音检测裁剪出供 whisper.cpp 使用的短音频。
...
```text
apps/api/storage/videos/{md5}/audio-raw.wav
apps/api/storage/videos/{md5}/audio.wav
apps/api/storage/videos/{md5}/audio-vad.wav
apps/api/storage/videos/{md5}/audio-vad-map.json
...
- Java 调用系统子进程。
- `audio-raw.wav` 保留完整音频，作为回退和诊断依据。
- `audio.wav` / `audio-vad.wav` 默认只保留非静音有效片段，降低 Whisper 输入时长。
- `audio-vad-map.json` 记录裁剪后时间轴到原视频时间轴的映射。
- VAD 检测失败、收益过小或没有可用片段时自动回退到完整音频。
- 设置执行超时。
- 捕获标准输出和错误日志。
- 失败时写入任务失败状态。
...
```powershell
Get-ChildItem E:\video\apps\api\storage\videos -Recurse -Filter audio-raw.wav
Get-ChildItem E:\video\apps\api\storage\videos -Recurse -Filter audio.wav
Get-ChildItem E:\video\apps\api\storage\videos -Recurse -Filter audio-vad.wav
Get-ChildItem E:\video\apps\api\storage\videos -Recurse -Filter audio-vad-map.json
Get-ChildItem E:\video\apps\api\storage\videos -Recurse -Filter ffmpeg.log
```

### 4.5 whisper.cpp ASR 字幕生成

后端调用本地 whisper.cpp 对 `audio-vad.wav` 转写；如果 VAD 回退，则 `audio-vad.wav` 与完整音频一致。ASR 结果入库前会按 `audio-vad-map.json` 映射回原视频时间轴。
...
```text
这是一个 Java 后端主导的长视频 AI 解析 1.0 版本，已经跑通真实上传、ASR、字幕、总结、SSE、MySQL、Redis、Qdrant 外部向量库、Embedding Provider、DeepSeek Chat、向量召回、本地 rerank、单视频 Agent 和多视频知识库问答。外部 BGE reranker、相似问题语义缓存、真实 LLM tool calling、RocketMQ 和多用户权限是 2.0 演进点，我会在面试中明确区分已实现能力和架构升级方向。
```


## 15. ASR VAD 提速补充

当前 ASR 链路已经加入 VAD/静音切片：`audio-raw.wav` 作为完整备份，`audio.wav` / `audio-vad.wav` 只保留有效人声片段，`audio-vad-map.json` 负责把 Whisper 输出重新映射回原视频时间轴。

黑盒验证：
1. 上传包含长停顿的视频 -> 验证: 视频目录存在完整 `audio-raw.wav`，且 `audio.wav` / `audio-vad.wav` 明显更短。
2. ASR 执行 -> 验证: `ASR_TRANSCRIBING` 阶段耗时相对完整音频下降。
3. 点击字幕 -> 验证: 播放器仍跳回原视频的真实时间戳。
4. 如果静音检测没有得到可用片段 -> 验证: 后端自动回退到完整音频，任务不因 VAD 失败中断。

## 16. 备份索引

- 本次会话恢复文件保留在仓库根目录：`codex-session-019eb1b9-full-chat.md`、`codex-session-019eb1b9-index.md`、`codex-session-019eb1b9-recovered-chat.md`、`codex-session-019eb1b9-tool-timeline.md`。
- 运行时备份目录 `backups/` 和 `备份/` 仍保持本地，不作为公开发布产物直接提交。
- 本地备份目录的公开索引见 `docs/v2.0/local-backup-manifest.md`。
- 需要继续接续时，优先读取 `README.md`、`apps/api/README.md`、`apps/api/CODEX.md` 和 `docs/v2.0/continuation-backup.md`。
